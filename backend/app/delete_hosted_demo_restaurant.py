"""
Delete the hosted Nadha Demo Cafe restaurant and only its data.

WARNING: this command deletes hosted production data.

Run from backend/ only when intentionally deleting the hosted demo:
    ALLOW_HOSTED_DEMO_DELETE=true PYTHONPATH=. venv/bin/python -m app.delete_hosted_demo_restaurant
"""
...
CONFIRMATION_TEXT = "DELETE HOSTED NADHA DEMO CAFE"
...
def _refuse_unless_hosted_delete_allowed() -> None:
    if os.environ.get("ALLOW_HOSTED_DEMO_DELETE") != "true":
        print(
            "Refusing to run: set ALLOW_HOSTED_DEMO_DELETE=true to delete hosted demo data.",
            file=sys.stderr,
        )
        sys.exit(2)


def _refuse_local_database() -> str:
    host = _database_host()
    if host in LOCAL_HOSTS:
        print(
            "Refusing to run: this hosted deletion command must not target "
            f"a local database. Configured host: {host}",
            file=sys.stderr,
        )
        sys.exit(2)
    if not host:
        print("Refusing to run: database hostname could not be determined.", file=sys.stderr)
        sys.exit(2)
    return host
...
def _delete_restaurant_data(db: Session, restaurant_id: int) -> dict[str, int]:
    ...
    # Delete dependents first so nullable and restrictive FKs cannot leave strays.
...
def main() -> None:
    _refuse_unless_hosted_delete_allowed()
    database_host = _refuse_local_database()

    print("WARNING: this command deletes hosted production data.")
    print(f"Database hostname: {database_host}")
    ...
    try:
        ...
        with db.begin():
            ...
            if locked_restaurant is None:
                raise RuntimeError("Hosted demo restaurant disappeared before deletion.")
            deleted_counts = _delete_restaurant_data(db, restaurant_id)

        _print_counts("Deleted records:", deleted_counts)
        print("\nCompleted deletion of hosted Nadha Demo Cafe data.")
