...
def cinema_data():
    ...
    db.add_all([cinema, restaurant]); db.flush()
    ...
    db.add_all([cinema_owner, restaurant_owner, category]); db.flush()
    ...
    yield {"cinema": cinema, "restaurant": restaurant, "item": item, "cinema_token": cinema_token, "restaurant_token": restaurant_token}
    db = SessionLocal(); db.query(Restaurant).filter(Restaurant.id.in_([cinema.id, restaurant.id])).delete(synchronize_session=False); db.commit(); db.close()
