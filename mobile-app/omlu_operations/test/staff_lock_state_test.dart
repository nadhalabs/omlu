...
void main() {
  ...
  setUp(() {
    ...
    session = RoleSession(
      ...
      tenantScope: const FlutterTenantScope(
        restaurantId: 1,
        actorId: 42,
        role: StaffRole.staff,
        ...
        if (request.uri.path.endsWith('/auth/staff/me')) {
          ...
          return const ApiResponse(
            ...
            body: {
              ...
              'restaurant_name': 'Nadha Cafe',
              'restaurant_slug': 'nadha-cafe',
              'scope': {
                'restaurant_id': 1,
                'actor_id': 10,
                'role': 'staff',
    ...
    repository = testAuthRepository(apiClient, MemoryTokenStorage());
  });

  Future<ProviderContainer> pumpGate(WidgetTester tester) async {
    final dependencies = testAuthenticatedCache(scope: session.tenantScope);
  ...
  RealtimeEvent event(
    ...
    type: type,
    timestamp: DateTime(2026, 7, 21, 23, 5),
  ...
  testWidgets(
    'closed restaurant has dedicated copy and back cannot reveal Staff UI',
    (tester) async {
      final container = await pumpGate(tester);
      container
          .read(staffAccessProvider.notifier)
          .handleEvent(
            event('restaurant.status_changed', state: {'status': 'closed'}),
          );
      await tester.pump();
  ...
  testWidgets('unlock waits for refresh then restores Tables with a toast', (
    tester,
  ) async {
    final container = await pumpGate(tester);
  ...
  testWidgets(
    'revoked session refresh routes to Login instead of lock screen',
    (tester) async {
      final container = ProviderContainer(
        overrides: [
          apiClientProvider.overrideWithValue(apiClient),
      ...
      await tester.pumpWidget(
        UncontrolledProviderScope(
          container: container,
          child: const OmluNativeApp(),
        ),
      );
      await tester.pumpAndSettle();
      container
          .read(staffAccessProvider.notifier)
          .handleEvent(event('staff.all_locked'));
      await tester.pump();
      revoked = true;
