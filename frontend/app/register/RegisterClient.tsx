...
export default function RegisterClient({ venueType }: { venueType: "restaurant" | "cinema" }) {
  const router = useRouter();
  const [form, setForm] = useState<RestaurantRegistrationRequest>({ ...initialForm, venue_type: venueType });
  const venueLabel = venueType === "cinema" ? "Cinema" : "Restaurant";
      ...
      <main className="mx-auto w-full max-w-4xl">
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-[var(--omlu-text-secondary)]">{venueLabel} workspace</p>
            <h1 className="mt-2 text-3xl font-black tracking-tight">Create {venueLabel}</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-6 shadow-sm">
          <div className="grid gap-8 lg:grid-cols-2">
            <section>
              <h2 className="text-lg font-black">{venueLabel}</h2>
              <div className="mt-5 grid gap-4">
                <TextField name="restaurant_name" label={`${venueLabel} name`} value={form.restaurant_name} error={fieldErrors.restaurant_name} disabled={loading} onChange={(value) => setField("restaurant_name", value)} autoComplete="organization" />
                <TextField name="restaurant_slug" label="Venue username" value={form.restaurant_slug} error={fieldErrors.restaurant_slug} disabled={loading} onChange={(value) => setField("restaurant_slug", value.toLowerCase())} placeholder={venueType === "cinema" ? "omlu-cinemas" : "nadha-cafe"} helperText="Used to sign in to your venue and may appear in ordering links." autoComplete="organization" />
            ...
            <section>
              ...
              <div className="mt-5 grid gap-4">
                ...
                <TextField name="owner_username" label="Owner username" value={form.owner_username} error={fieldErrors.owner_username} disabled={loading} onChange={(value) => setField("owner_username", value.toLowerCase())} placeholder="anjali" helperText="This username only needs to be unique within your venue." autoComplete="username" />
          ...
          <label className="mt-6 flex items-start gap-3 text-sm font-semibold text-[var(--omlu-text-primary)]">
            ...
            />
            <span>
              I confirm that I’m authorized to create this {venueLabel.toLowerCase()} account and agree to OMLU’s{" "}
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            >
              {loading ? `Creating ${venueLabel.toLowerCase()}…` : `Create ${venueLabel}`}
            </button>
