...
export default function StaffAvailabilityClient() {
  ...
  return (
    <div className="min-h-screen bg-[var(--omlu-page-background)] px-4 py-8 text-[var(--omlu-text-secondary)]">
      <div className="mx-auto flex max-w-6xl flex-col gap-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <Link href="/staff" className="text-sm font-bold text-orange-400">Back to staff home</Link>
            <h1 className="mt-2 text-3xl font-black text-[var(--omlu-text-primary)]">Availability</h1>
            <p className="mt-1 text-[11px] font-bold uppercase tracking-wide text-[var(--omlu-text-secondary)]">Real-time: {realtimeStatus}</p>
          </div>
          <Link href="/staff/tables" className="rounded-lg bg-orange-600 px-4 py-3 text-sm font-black text-[var(--omlu-primary-action-text)]">New Order</Link>
        </div>
        <section className="grid gap-3 md:grid-cols-[1fr_220px]">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search menu" className="rounded-lg border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] px-4 py-4 text-[var(--omlu-text-primary)] outline-none focus:border-orange-500" />
          <select value={categoryId} onChange={(event) => setCategoryId(event.target.value === "all" ? "all" : Number(event.target.value))} className="rounded-lg border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] px-4 py-4 text-[var(--omlu-text-primary)] outline-none focus:border-orange-500">
        ...
        {loading ? <div className="text-sm text-[var(--omlu-text-secondary)]">Loading availability...</div> : items.length === 0 ? (
          <div className="rounded-xl border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] p-8 text-center text-sm text-[var(--omlu-text-secondary)]">No menu items found.</div>
        ) : (
          <div className="grid gap-3">
            {items.map((item) => (
              <div key={item.id} className="rounded-xl border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <div className="font-black text-[var(--omlu-text-primary)]">{item.name_en}</div>
                    <div className="text-xs font-bold uppercase tracking-wider text-[var(--omlu-text-secondary)]">{item.category_name}</div>
                  </div>
                ...
                {item.option_groups.length > 0 && (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {item.option_groups.map((group) => (
                      <div key={group.id} className="rounded-lg bg-[var(--omlu-page-background)] p-3">
                        <div className="text-sm font-black text-[var(--omlu-text-secondary)]">{group.name}</div>
