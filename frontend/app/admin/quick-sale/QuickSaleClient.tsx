  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    {saleType && <section className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <label className="relative mt-4 block w-full"><span className="sr-only">Search menu</span><span aria-hidden="true" className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-base text-zinc-400">⌕</span><input aria-label="Search menu items" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search menu items..." className={`h-12 w-full rounded-xl border border-zinc-300 bg-zinc-50 py-2 pl-11 text-sm text-zinc-950 outline-none placeholder:text-zinc-400 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 ${search ? "pr-16" : "pr-4"}`} />{search && <button type="button" onClick={() => setSearch("")} className="absolute right-3 top-1/2 min-h-10 -translate-y-1/2 rounded-lg px-2 text-xs font-bold text-zinc-500 hover:text-zinc-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">Clear</button>}</label>
