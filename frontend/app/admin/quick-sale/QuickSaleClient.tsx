...
type QuickSale = { order_number: string; public_token: string; sale_type: SaleType; status: string; note: string | null; reason: string | null; subtotal: string; discount_amount: string; taxable_amount: string | null; gst_enabled: boolean; gst_rate: string | null; cgst_amount: string | null; sgst_amount: string | null; igst_amount: string | null; tax_amount: string; total: string; grand_total: string; payment_method: PaymentMethod | null; customer_gstin: string | null; customer_legal_name: string | null; customer_billing_address: string | null; customer_state_name: string | null; customer_state_code: string | null; entered_by_name: string; entered_by_role: string; created_at: string; completed_at: string | null; items: SaleItem[] };
...
export default function QuickSaleClient() {
  ...
  const customerGstPayload = useMemo(() => customerGst ? {
    ...
    customer_billing_address: customerGst.billingAddress,
    customer_state_name: customerGst.state,
    customer_state_code: customerGst.stateCode,
    place_of_supply_code: customerGst.stateCode,
  } : {}, [customerGst]);
  useEffect(() => {
  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
    <header>
      ...
      <p className="mt-1 text-sm text-[var(--omlu-text-secondary)]">Counter takeaway and unrecorded completed sales—without a table or dining session.</p>
      <p className="mt-3 text-xs font-semibold text-[var(--omlu-text-secondary)]" aria-label="Quick Sale steps">Choose type <span aria-hidden="true">→</span> Add items <span aria-hidden="true">→</span> Review <span aria-hidden="true">→</span> Confirm</p>
    </header>
    ...
    <section className="rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-4 shadow-sm" aria-labelledby="sale-type-heading">
      <h2 id="sale-type-heading" className="mb-3 text-sm font-black text-[var(--omlu-text-primary)]">Sale type</h2>
      <div className="grid grid-cols-2 gap-2" role="group" aria-label="Sale type">
        {([{"value": "takeaway", "label": "Takeaway", "helper": "Food still needs preparation"}, {"value": "late_entry", "label": "Late Entry", "helper": "Food was already served or handed over"}] as const).map((mode) => {
          ...
          return <button key={mode.value} type="button" aria-pressed={active} onClick={() => setSaleType(mode.value)} className={`min-h-20 rounded-xl border-2 px-3 py-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 ${active ? "border-orange-500 bg-orange-50 text-orange-950 shadow-sm dark:bg-orange-950/30 dark:text-[var(--omlu-text-primary)]" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] hover:border-orange-300 hover:bg-orange-50/40 dark:hover:border-orange-500/50 dark:hover:bg-orange-950/10"}`}><span className="block text-sm font-black sm:text-base">{mode.label}</span><span className={`mt-1 block text-[11px] leading-4 sm:text-xs ${active ? "text-orange-800 dark:text-orange-300" : "text-[var(--omlu-text-secondary)]"}`}>{mode.helper}</span></button>;
        })}
      </div>
    </section>

    {saleType && <section className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
      <div className="rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-4 shadow-sm sm:p-5">
        <div><p className="text-xs font-bold uppercase tracking-wide text-orange-600">Step 2</p><h2 className="mt-1 text-xl font-black text-[var(--omlu-text-primary)]">Add items</h2></div>
        <label className="relative mt-4 block w-full"><span className="sr-only">Search menu</span><span aria-hidden="true" className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-base text-[var(--omlu-text-secondary)]">⌕</span><input aria-label="Search menu items" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search menu items..." className={`h-12 w-full rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] py-2 pl-11 text-sm text-[var(--omlu-text-primary)] outline-none placeholder:text-[var(--omlu-text-secondary)] focus:border-orange-500 focus:ring-2 focus:ring-orange-100 ${search ? "pr-16" : "pr-4"}`} />{search && <button type="button" onClick={() => setSearch("")} className="absolute right-3 top-1/2 min-h-10 -translate-y-1/2 rounded-lg px-2 text-xs font-bold text-[var(--omlu-text-secondary)] hover:text-[var(--omlu-text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">Clear</button>}</label>
        <div className="mt-4 grid gap-3 xl:grid-cols-2">{visibleMenu.map((item) => {
          ...
          return <div key={item.id} className="flex min-h-28 flex-col justify-between gap-4 rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-4 transition hover:border-[var(--omlu-border-strong)] hover:shadow-sm">
            <div className="flex items-start justify-between gap-3"><div><div className="font-black text-[var(--omlu-text-primary)]">{item.name}</div><div className="mt-1 text-xs font-medium text-[var(--omlu-text-secondary)]">{item.has_options ? "Starting from" : "Price"} ₹{item.price}</div></div>{itemQuantity > 0 && <span className="rounded-full bg-orange-100 px-2 py-1 text-xs font-black text-orange-700">{itemQuantity} added</span>}</div>
            <div className="flex items-center justify-between gap-3">
              {item.has_options ? <button type="button" onClick={() => openOptions(item)} className="min-h-10 rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] px-3 text-sm font-black text-[var(--omlu-text-primary)] hover:border-orange-400 hover:text-orange-700">Select options</button> : <span className="text-xs text-[var(--omlu-text-secondary)]">Tap + to add</span>}
              <div className="flex items-center rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-1 shadow-sm">
                {!item.has_options && <button type="button" aria-label={`Remove ${item.name}`} onClick={() => { const index = cart.findIndex((line) => line.menu_item_id === item.id); if (index >= 0) setLineQuantity(index, cart[index].quantity - 1); }} disabled={itemQuantity === 0} className="h-9 w-9 rounded-md text-lg font-bold text-[var(--omlu-text-primary)] hover:bg-[var(--omlu-muted-surface)] disabled:cursor-not-allowed disabled:text-[var(--omlu-text-secondary)]">−</button>}
                <span className="w-9 text-center text-sm font-black text-[var(--omlu-text-primary)]">{itemQuantity}</span>
                <button type="button" aria-label={item.has_options ? `Select options for ${item.name}` : `Add ${item.name}`} onClick={() => item.has_options ? openOptions(item) : addSimpleItem(item)} className="h-9 w-9 rounded-md bg-orange-600 text-lg font-black text-[var(--omlu-primary-action-text)] hover:bg-orange-500">+</button>
              </div>
            </div>
          </div>;
        })}</div>
        {visibleMenu.length === 0 && <div className="mt-4 rounded-xl border border-dashed border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-10 text-center"><p className="font-black text-[var(--omlu-text-primary)]">No matching items</p><p className="mt-1 text-sm text-[var(--omlu-text-secondary)]">Try a different item name.</p></div>}
      </div>
      <aside className="rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-4 text-[var(--omlu-text-primary)] shadow-sm sm:p-5 lg:sticky lg:top-6">
        <div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-orange-600">Steps 3–5</p><h2 className="mt-1 text-xl font-black">Order summary</h2></div>{cart.length > 0 && <span className="rounded-full bg-[var(--omlu-muted-surface)] px-2.5 py-1 text-xs font-black text-[var(--omlu-text-secondary)]">{cart.reduce((sum, line) => sum + line.quantity, 0)} items</span>}</div>
        {cart.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] px-5 py-10 text-center"><div className="text-3xl" aria-hidden="true">＋</div><p className="mt-3 font-black text-[var(--omlu-text-primary)]">No items added yet</p><p className="mt-1 text-sm text-[var(--omlu-text-secondary)]">Add items from the menu to start this sale.</p></div> : <div className="mt-4 space-y-3 text-sm">{cart.map((line, index) => <div key={`${line.menu_item_id}-${optionSignature(line.selected_options)}-${index}`} className="rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-3 text-[var(--omlu-text-primary)]">
          <div className="flex justify-between gap-3"><div><span className="font-black text-[var(--omlu-text-primary)]">{line.item_name}</span>{optionLabels(line).length > 0 && <p className="mt-1 text-xs text-[var(--omlu-text-secondary)]">{optionLabels(line).join(" · ")}</p>}</div><span className="font-bold text-[var(--omlu-text-primary)]">₹{(Number(line.unit_price) * line.quantity).toFixed(2)}</span></div>
          <div className="mt-3 flex items-center justify-between gap-2"><div className="flex items-center rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-0.5"><button type="button" aria-label={`Decrease ${line.item_name}`} onClick={() => setLineQuantity(index, line.quantity - 1)} className="h-8 w-8 rounded-md font-bold text-[var(--omlu-text-primary)] hover:bg-[var(--omlu-muted-surface)]">−</button><span className="w-8 text-center font-black text-[var(--omlu-text-primary)]">{line.quantity}</span><button type="button" aria-label={`Increase ${line.item_name}`} onClick={() => setLineQuantity(index, line.quantity + 1)} className="h-8 w-8 rounded-md font-bold text-[var(--omlu-text-primary)] hover:bg-[var(--omlu-muted-surface)]">+</button></div>{line.selected_options.length > 0 && <button type="button" onClick={() => { const item = data?.menu_items.find((candidate) => candidate.id === line.menu_item_id); if (item) openOptions(item, index); }} className="rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] px-2.5 py-2 text-xs font-black text-[var(--omlu-text-primary)] hover:border-orange-400 hover:text-orange-700">Edit options</button>}</div>
        </div>)}</div>}
        <div className="mt-4 space-y-2 border-t border-[var(--omlu-border-strong)] pt-4 text-sm">
          {previewLoading && <div className="text-[var(--omlu-text-secondary)]" role="status">Calculating authoritative total…</div>}
          {previewError && <div className="rounded-lg bg-red-50 p-2 text-red-700" role="alert">{previewError}</div>}
          {preview && <>
            <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>Subtotal</span><span className="font-semibold">₹{preview.subtotal}</span></div>
            {Number(preview.discount_amount) !== 0 && <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>Discount</span><span>−₹{preview.discount_amount}</span></div>}
            {preview.gst_enabled && preview.tax_mode === "inclusive" && <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>Includes GST</span><span>₹{preview.tax_amount}</span></div>}
            {preview.gst_enabled && Number(preview.igst_amount) !== 0 && <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>IGST {preview.igst_rate}%</span><span>₹{preview.igst_amount}</span></div>}
            {preview.gst_enabled && Number(preview.igst_amount) === 0 && <>
              <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>CGST {preview.cgst_rate}%</span><span>₹{preview.cgst_amount}</span></div>
              <div className="flex justify-between text-[var(--omlu-text-secondary)]"><span>SGST {preview.sgst_rate}%</span><span>₹{preview.sgst_amount}</span></div>
            </>}
        ...
        <div className="mt-4 rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-3">
          ...
          <button disabled={saving || previewLoading || !preview || !cart.length} onClick={submit} className="mt-3 min-h-12 w-full rounded-xl bg-orange-600 px-3 font-black text-[var(--omlu-primary-action-text)] shadow-sm hover:bg-orange-500 disabled:cursor-not-allowed disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)]">{saving ? "Saving…" : previewLoading ? "Calculating total…" : preview ? `${saleType === "takeaway" ? "Send to Kitchen" : "Record Completed Sale"} — ₹${preview.grand_total}` : saleType === "takeaway" ? "Send to Kitchen" : "Record Completed Sale"}</button>
        </div>
      </aside>
    </section>}

    {customisingItem && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-3 sm:items-center sm:p-4">
      <div ref={optionsDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="quick-sale-options-title" aria-describedby="quick-sale-options-instruction" className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-hidden rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] shadow-2xl outline-none sm:max-h-[90vh]">
        <div className="flex items-start justify-between px-5 pb-3 pt-5"><div><h2 id="quick-sale-options-title" className="text-xl font-black text-[var(--omlu-text-primary)]">{customisingItem.name}</h2><p id="quick-sale-options-instruction" className="mt-1 text-sm text-[var(--omlu-text-secondary)]">Select the required options</p></div><button type="button" aria-label="Close specifications" onClick={() => setCustomisingItem(null)} className="flex h-10 w-10 items-center justify-center rounded-lg text-xl text-[var(--omlu-text-secondary)] hover:bg-[var(--omlu-muted-surface)] hover:text-[var(--omlu-text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">×</button></div>
        <div className="max-h-[55vh] overflow-y-auto px-5 pb-2">{customisingItem.option_groups.map((group) => {
          ...
          return <section key={group.id} className="mb-4"><div className="flex justify-between gap-3"><div><h3 className="font-black text-[var(--omlu-text-primary)]">{group.name}</h3><p className="mt-0.5 text-xs text-[var(--omlu-text-secondary)]">{minimum ? `Choose ${minimum}` : "Optional"}{group.maximum_selections && group.maximum_selections !== minimum ? ` · up to ${group.maximum_selections}` : ""}</p></div>{minimum > 0 && <span className={`h-fit rounded-full px-2 py-1 text-[11px] font-black ${selectedCount < minimum ? "bg-orange-100 text-orange-800" : "bg-[var(--omlu-muted-surface)] text-[var(--omlu-text-secondary)]"}`}>{selectedCount < minimum ? "Required" : "Complete"}</span>}</div><div className="mt-3 grid gap-2" role={multi ? "group" : "radiogroup"} aria-label={group.name}>{group.options.map((option) => {
            ...
            return <button key={option.id} type="button" role={multi ? undefined : "radio"} aria-checked={multi ? undefined : checked} aria-pressed={multi ? checked : undefined} disabled={disabled} onClick={() => toggleDraftOption(group.id, option.id, multi)} className={`flex min-h-12 items-center justify-between gap-3 rounded-xl border-2 px-3 py-3 text-left text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:border-[var(--omlu-border-strong)] disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)] ${checked ? "border-orange-500 bg-orange-50 text-orange-950 dark:border-orange-500 dark:bg-orange-950/40 dark:text-[var(--omlu-text-primary)]" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] hover:border-orange-300 dark:hover:border-orange-500/50"}`}><span className="flex min-w-0 items-center gap-3"><span aria-hidden="true" className={`flex h-5 w-5 shrink-0 items-center justify-center ${multi ? "rounded-md" : "rounded-full"} border-2 ${checked ? "border-orange-600 bg-orange-600 text-[10px] text-white" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-transparent"}`}>✓</span><span className="font-bold">{option.name}</span></span><span className="shrink-0 font-extrabold text-[var(--omlu-text-primary)]">{group.type === "variant" ? `₹${Number(option.price_delta).toFixed(2)}` : `+₹${Number(option.price_delta).toFixed(2)}`}</span></button>;
          })}</div></section>;
        })}</div>
        <div className="border-t border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-5"><div className="mb-4 flex items-center justify-between"><span className="text-sm font-black text-[var(--omlu-text-primary)]">Quantity</span><div className="flex items-center rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-1"><button type="button" aria-label="Decrease quantity" disabled={draftQuantity <= 1} onClick={() => setDraftQuantity((value) => Math.max(1, value - 1))} className="h-10 w-10 rounded-lg text-lg font-black text-[var(--omlu-text-primary)] hover:bg-[var(--omlu-muted-surface)] disabled:cursor-not-allowed disabled:text-[var(--omlu-text-secondary)]">−</button><span className="w-10 text-center font-black text-[var(--omlu-text-primary)]" aria-live="polite">{draftQuantity}</span><button type="button" aria-label="Increase quantity" disabled={draftQuantity >= 50} onClick={() => setDraftQuantity((value) => Math.min(50, value + 1))} className="h-10 w-10 rounded-lg text-lg font-black text-[var(--omlu-text-primary)] hover:bg-[var(--omlu-muted-surface)] disabled:cursor-not-allowed disabled:text-[var(--omlu-text-secondary)]">+</button></div></div>{!requiredSelectionsComplete(customisingItem, selectedOptionsFromDraft()) && <p className="mb-2 text-sm font-bold text-orange-800">Select all required options to continue.</p>}<button type="button" disabled={!requiredSelectionsComplete(customisingItem, selectedOptionsFromDraft())} onClick={() => {
        ...
        }} className="h-12 w-full rounded-xl bg-orange-600 font-black text-[var(--omlu-primary-action-text)] hover:bg-orange-500 disabled:cursor-not-allowed disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)]">{editingIndex === null ? "Add to order" : "Update order"} — ₹{(optionPrice(customisingItem, selectedOptionsFromDraft()) * draftQuantity).toFixed(2)}</button></div>
      </div>
    </div>}
