    ...
    <section className="rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-4 shadow-sm" aria-labelledby="sale-type-heading">
      ...
      <div className="grid grid-cols-2 gap-2" role="group" aria-label="Sale type">
        {([{"value": "takeaway", "label": "Takeaway", "helper": "Food still needs preparation"}, {"value": "late_entry", "label": "Late Entry", "helper": "Food was already served or handed over"}] as const).map((mode) => {
          ...
          return <button key={mode.value} type="button" aria-pressed={active} onClick={() => setSaleType(mode.value)} className={`relative min-h-20 rounded-xl border-2 px-3 py-3 pr-10 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 ${active ? "border-orange-500 bg-orange-50 text-orange-950 shadow-sm" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] hover:border-orange-300 hover:bg-orange-50/40"}`}><span className="block text-sm font-black sm:text-base">{mode.label}</span><span className={`mt-1 block text-[11px] leading-4 sm:text-xs ${active ? "text-orange-800" : "text-[var(--omlu-text-secondary)]"}`}>{mode.helper}</span><span aria-hidden="true" className={`absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full border text-xs font-black ${active ? "border-orange-500 bg-orange-600 text-[var(--omlu-text-primary)]" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-transparent"}`}>✓</span></button>;
        })}
      </div>
    </section>
          ...
          return <div key={item.id} className="flex min-h-28 flex-col justify-between gap-4 rounded-xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-muted-surface)] p-4 transition hover:border-[var(--omlu-border-strong)] hover:shadow-sm">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center rounded-lg border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] p-1 shadow-sm">
                ...
                <button type="button" aria-label={item.has_options ? `Select options for ${item.name}` : `Add ${item.name}`} onClick={() => item.has_options ? openOptions(item) : addSimpleItem(item)} className="h-9 w-9 rounded-md bg-orange-600 text-lg font-black text-[var(--omlu-text-primary)] hover:bg-orange-500">+</button>
              </div>
            </div>
          </div>;
        })}</div>
      ...
      <div ref={optionsDialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="quick-sale-options-title" aria-describedby="quick-sale-options-instruction" className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-hidden rounded-2xl border border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] shadow-2xl outline-none sm:max-h-[90vh]">
        ...
        <div className="max-h-[55vh] overflow-y-auto px-5 pb-2">{customisingItem.option_groups.map((group) => {
          ...
          return <section key={group.id} className="mb-4"><div className="flex justify-between gap-3"><div><h3 className="font-black text-[var(--omlu-text-primary)]">{group.name}</h3><p className="mt-0.5 text-xs text-[var(--omlu-text-secondary)]">{minimum ? `Choose ${minimum}` : "Optional"}{group.maximum_selections && group.maximum_selections !== minimum ? ` · up to ${group.maximum_selections}` : ""}</p></div>{minimum > 0 && <span className={`h-fit rounded-full px-2 py-1 text-[11px] font-black ${selectedCount < minimum ? "bg-orange-100 text-orange-800" : "bg-[var(--omlu-muted-surface)] text-[var(--omlu-text-secondary)]"}`}>{selectedCount < minimum ? "Required" : "Complete"}</span>}</div><div className="mt-3 grid gap-2" role={multi ? "group" : "radiogroup"} aria-label={group.name}>{group.options.map((option) => {
            ...
            return <button key={option.id} type="button" role={multi ? undefined : "radio"} aria-checked={multi ? undefined : checked} aria-pressed={multi ? checked : undefined} disabled={disabled} onClick={() => toggleDraftOption(group.id, option.id, multi)} className={`flex min-h-12 items-center justify-between gap-3 rounded-xl border-2 px-3 py-3 text-left text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:border-[var(--omlu-border-strong)] disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)] ${checked ? "border-orange-500 bg-orange-50 text-[var(--omlu-text-primary)]" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-[var(--omlu-text-primary)] hover:border-orange-300"}`}><span className="flex min-w-0 items-center gap-3"><span aria-hidden="true" className={`flex h-5 w-5 shrink-0 items-center justify-center ${multi ? "rounded-md" : "rounded-full"} border-2 ${checked ? "border-orange-600 bg-orange-600 text-[10px] text-[var(--omlu-text-primary)]" : "border-[var(--omlu-border-strong)] bg-[var(--omlu-primary-surface)] text-transparent"}`}>✓</span><span>{option.name}</span></span><span className="shrink-0 text-[var(--omlu-text-primary)]">{group.type === "variant" ? `₹${Number(option.price_delta).toFixed(2)}` : `+₹${Number(option.price_delta).toFixed(2)}`}</span></button>;
          })}</div></section>;
        })}</div>
        ...
        }} className="h-12 w-full rounded-xl bg-orange-600 font-black text-[var(--omlu-text-primary)] hover:bg-orange-500 disabled:cursor-not-allowed disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)]">{editingIndex === null ? "Add to order" : "Update order"} — ₹{(optionPrice(customisingItem, selectedOptionsFromDraft()) * draftQuantity).toFixed(2)}</button></div>
      </div>
    </div>}
