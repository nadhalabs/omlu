...
export default function PendingPaymentsClient() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    {loading ? <div className="h-48 animate-pulse rounded-2xl bg-zinc-800" /> : items.length === 0 ?
      ...
      <div className="grid gap-4 xl:grid-cols-2">{items.map((item) => <article id={`bill-${item.bill_number}`} key={item.bill_id} className={`min-w-0 rounded-2xl border bg-zinc-950 p-5 ${selectedBill === item.bill_number ? "border-orange-500 ring-2 ring-orange-500/20" : "border-zinc-800"}`}>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <button onClick={() => void openPaymentDialog(item, "counter_cash")} className="rounded-xl bg-emerald-700 px-4 py-2 text-sm font-black text-white">Confirm Cash received</button>
          <button onClick={() => void openPaymentDialog(item, "counter_upi")} className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-black text-white">Confirm UPI received</button>
        </div>
      </article>)}</div>}
  </div>;
}
