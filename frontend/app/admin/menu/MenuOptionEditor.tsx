...
export default function MenuOptionEditor({ itemId, itemName }: Props) {
  ...
  return <section className="rounded-2xl border border-zinc-700 bg-zinc-950 p-4">
    <h4 className="font-black text-white">Options & specifications</h4>
    <p className="mt-1 text-xs text-zinc-400">Variants use a final customer price. Add-ons use an amount added to the base price.</p>
    {message && <p className="mt-2 text-xs font-bold text-amber-300">{message}</p>}
    <div className="mt-3 space-y-3">
      {groups.map((group) => <div key={group.id} className="rounded-xl border border-zinc-800 p-3">
        <div className="grid gap-2 sm:grid-cols-4">
          <input aria-label="Option-group name" value={group.name} onChange={(event) => updateGroup(group.id, { name: event.target.value })} className="rounded border border-zinc-700 bg-zinc-900 p-2 text-white" />
          <select value={group.type} onChange={(event) => updateGroup(group.id, { type: event.target.value as "variant" | "addon", maximum_selections: event.target.value === "variant" ? 1 : group.maximum_selections })} className="rounded border border-zinc-700 bg-zinc-900 p-2 text-white"><option value="variant">Single-select / final price</option><option value="addon">Multi-select / price adjustment</option></select>
          <label className="text-xs text-zinc-300"><input type="checkbox" checked={group.required} onChange={(event) => updateGroup(group.id, { required: event.target.checked })} /> Required</label>
          <label className="text-xs text-zinc-300">Sort <input type="number" min="0" value={group.display_order} onChange={(event) => updateGroup(group.id, { display_order: Number(event.target.value) })} className="w-16 rounded bg-zinc-900 p-1" /></label>
          <label className="text-xs text-zinc-300">Min <input type="number" min="0" value={group.minimum_selections} onChange={(event) => updateGroup(group.id, { minimum_selections: Number(event.target.value) })} className="w-16 rounded bg-zinc-900 p-1" /></label>
          <label className="text-xs text-zinc-300">Max <input type="number" min="0" disabled={group.type === "variant"} value={group.type === "variant" ? 1 : group.maximum_selections} onChange={(event) => updateGroup(group.id, { maximum_selections: Number(event.target.value) })} className="w-16 rounded bg-zinc-900 p-1 disabled:bg-zinc-800 disabled:text-zinc-400" /></label>
        </div>
        ...
        <button type="button" disabled={saving} onClick={() => void saveExisting(group)} className="mt-3 rounded-lg bg-zinc-700 px-3 py-2 text-xs font-black text-white disabled:bg-zinc-800 disabled:text-zinc-400">Save group</button>
      </div>)}
    </div>
    <div className="mt-4 rounded-xl border border-dashed border-zinc-700 p-3">
      ...
      <div className="mt-2 grid gap-2 sm:grid-cols-3">
        <input placeholder="Group name" value={name} onChange={(event) => setName(event.target.value)} className="rounded border border-zinc-700 bg-zinc-900 p-2 text-white" />
        <select value={type} onChange={(event) => { const next = event.target.value as "variant" | "addon"; setType(next); if (next === "variant") setMaximum(1); }} className="rounded border border-zinc-700 bg-zinc-900 p-2 text-white"><option value="variant">Single-select / final price</option><option value="addon">Multi-select / adjustment</option></select>
        <label className="text-xs text-zinc-300"><input type="checkbox" checked={required} onChange={(event) => setRequired(event.target.checked)} /> Required</label>
        <label className="text-xs text-zinc-300">Min <input type="number" min="0" value={minimum} onChange={(event) => setMinimum(Number(event.target.value))} className="w-16 rounded bg-zinc-900 p-1" /></label>
        <label className="text-xs text-zinc-300">Max <input type="number" min="0" disabled={type === "variant"} value={maximum} onChange={(event) => setMaximum(Number(event.target.value))} className="w-16 rounded bg-zinc-900 p-1 disabled:bg-zinc-800 disabled:text-zinc-400" /></label>
        <label className="text-xs text-zinc-300">Sort <input type="number" min="0" value={displayOrder} onChange={(event) => setDisplayOrder(Number(event.target.value))} className="w-16 rounded bg-zinc-900 p-1" /></label>
      </div>
