  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/65" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div
        ...
        tabIndex={-1}
        className="flex h-full w-full max-w-md flex-col border-l border-[var(--omlu-border)] bg-[var(--omlu-page-background)] text-[var(--omlu-text-secondary)] shadow-2xl outline-none"
      >
        <div className="flex items-center justify-between border-b border-[var(--omlu-border)] px-5 py-4">
          <h2 id="availability-title" className="text-lg font-semibold">Manage availability</h2>
          <button onClick={onClose} aria-label="Close availability panel" className="rounded-lg px-3 py-2 text-sm text-[var(--omlu-text-secondary)] hover:bg-[var(--omlu-primary-surface)] hover:text-[var(--omlu-text-primary)]">Close</button>
        </div>
        <div className="border-b border-[var(--omlu-border)] p-4">
          <label htmlFor="availability-search" className="sr-only">Search menu items</label>
          <input id="availability-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search items…" className="w-full rounded-xl border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] px-4 py-3 text-sm outline-none focus:border-orange-500" />
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2">
          {loading && items.length === 0 ? <p className="py-6 text-center text-sm text-[var(--omlu-text-secondary)]">Loading items…</p> : null}
          {!loading && grouped.length === 0 ? <p className="py-6 text-center text-sm text-[var(--omlu-text-secondary)]">No matching items</p> : null}
          {grouped.map(([category, categoryItems]) => (
            <section key={category} className="py-3">
              <h3 className="mb-1 text-xs font-medium uppercase tracking-wide text-[var(--omlu-text-secondary)]">{category}</h3>
              <div className="space-y-2">{categoryItems.map((item) => {
                const pending = Boolean(pendingIds[item.id]);
                return (
                <div key={item.id} aria-busy={pending} className="rounded-xl border border-[var(--omlu-border)] bg-[var(--omlu-primary-surface)] p-4">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold text-[var(--omlu-text-secondary)]">{item.name_en}</p>
                    <p className="mt-0.5 truncate text-xs text-[var(--omlu-text-secondary)]">{item.category_name}</p>
                  </div>
                  <div className="mt-3 flex items-center justify-between gap-3">
                  <span className="text-xs font-medium text-[var(--omlu-text-secondary)]">Current state</span>
                  <button
                    aria-pressed={item.is_available}
                    aria-label={`${item.name_en}: ${item.is_available ? "Available" : "Unavailable"}`}
                    disabled={pending}
                    onClick={() => void update(item)}
                    className={`min-h-11 min-w-28 shrink-0 rounded-xl border px-3 text-xs font-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--omlu-page-background)] disabled:cursor-wait disabled:border-[var(--omlu-border)] disabled:bg-[var(--omlu-muted-surface)] disabled:text-[var(--omlu-text-secondary)] ${item.is_available ? "border-green-700 bg-green-950/50 text-green-300 hover:bg-green-900/60" : "border-red-800 bg-red-950/40 text-red-300 hover:bg-red-900/50"}`}
                  >
