...
export function StaffBottomNav({ active, requestCount }: StaffBottomNavProps) {
  ...
  const itemClass = (name: StaffBottomNavProps["active"]) =>
    `flex h-14 flex-1 flex-col items-center justify-center rounded-2xl text-xs font-bold transition ${
      active === name ? "text-orange-600" : "text-zinc-500"
    }`;
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-md px-4 pb-4 sm:max-w-xl" aria-label="Staff navigation">
      <div className="grid h-20 grid-cols-[1fr_88px_1fr] items-center rounded-[28px] border border-orange-100 bg-white/95 px-3 shadow-lg shadow-orange-100/70 backdrop-blur">
        ...
        <Link href="/staff/requests" className={`${itemClass("requests")} relative`}>
          ...
          <span>Requests</span>
          {pendingRequests > 0 && (
            <span className="absolute right-4 top-2 min-w-5 rounded-full bg-orange-600 px-1.5 py-0.5 text-center text-[10px] font-black text-white">
              {pendingRequests}
            </span>
          )}
        </Link>
