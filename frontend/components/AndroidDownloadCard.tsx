...
function DownloadLink({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <a
      ...
      className={`${compact ? "min-h-10 px-3 text-xs" : "min-h-12 w-full px-5 text-sm sm:w-auto"} inline-flex items-center justify-center gap-2 rounded-xl bg-orange-600 font-black text-white shadow-sm transition motion-safe:hover:-translate-y-0.5 hover:bg-orange-700 active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2 ${pressed ? "scale-[0.98]" : ""}`}
...
export function AndroidDownloadCard({ variant = "landing", dismissible = false, className = "" }: AndroidDownloadCardProps) {
  ...
  if (variant === "compact") {
    return (
      <aside className={`print:hidden rounded-xl border border-orange-900/40 bg-orange-950/20 p-3 text-zinc-100 ${className}`} aria-label="OMLU Android app">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            {dismissible && <button type="button" onClick={dismiss} className="min-h-10 rounded-lg px-3 text-xs font-bold text-zinc-400 hover:bg-zinc-800 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400">Dismiss</button>}
          </div>
        </div>
      </aside>
    );
      ...
      <div className="grid items-center gap-8 p-6 sm:p-9 lg:grid-cols-[1.35fr_0.65fr] lg:p-12">
        <div>
          ...
          <div className="mt-5 max-w-xl text-sm text-zinc-300">
            <button type="button" aria-expanded={helpOpen} aria-controls="omlu-apk-install-help" onClick={() => setHelpOpen((open) => !open)} className="rounded-md font-bold text-orange-400 outline-none focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950">How to install the APK <span aria-hidden="true">{helpOpen ? "−" : "+"}</span></button>
        ...
        <div className="hidden justify-center lg:flex" aria-hidden="true">
          <div className="relative h-64 w-40 rotate-2 rounded-[2.25rem] border-[6px] border-zinc-700 bg-zinc-900 p-3 shadow-2xl motion-safe:transition-transform motion-safe:hover:-translate-y-1">
            ...
            <div className="mt-12 flex flex-col items-center text-center"><span className="rounded-2xl bg-orange-600 p-4 text-white"><AndroidIcon className="h-9 w-9" /></span><span className="mt-4 text-xl font-black">OMLU</span><span className="mt-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500">Restaurant operations</span></div>
          </div>
        </div>
      </div>
    </section>
