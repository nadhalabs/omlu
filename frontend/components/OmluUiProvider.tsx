...
function OmluConfirmDialog({ state, close }: { state: State; close: (value: boolean | string | null) => void }) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-y-auto overscroll-contain bg-black/70 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))]" onMouseDown={(event) => { if (event.target === event.currentTarget) cancel(); }}><div ref={panel} role="dialog" aria-modal="true" aria-labelledby="omlu-dialog-title" aria-describedby="omlu-dialog-message" onKeyDown={onKeyDown} className="my-auto max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 p-5 text-zinc-100 shadow-2xl sm:p-6"><h2 id="omlu-dialog-title" className="break-words text-xl font-black text-white">{options.title}</h2><p id="omlu-dialog-message" className="mt-2 break-words text-sm leading-6 text-zinc-400">{options.message}</p>{options.details && <div className="mt-4 space-y-2 rounded-xl bg-zinc-950 p-4 text-sm">{options.details.map((detail) => <div key={detail} className="break-words font-bold text-zinc-300">{detail}</div>)}</div>}{state.kind === "input" && <label className="mt-4 block text-sm font-bold text-zinc-300">{state.options.label}<textarea value={value} onChange={(event) => setValue(event.target.value)} placeholder={state.options.placeholder} maxLength={state.options.maxLength || 1024} disabled={busy} className="mt-2 min-h-24 w-full rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-white outline-none focus:border-orange-500" /><span className="block text-right text-[10px] text-zinc-400">{value.length}/{state.options.maxLength || 1024}</span></label>}{error && <div role="alert" className="mt-4 rounded-xl border border-red-900/50 bg-red-950/30 p-3 text-sm font-bold text-red-300">{error}</div>}<div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">{state.kind !== "alert" && <button disabled={busy} onClick={cancel} className="min-h-11 rounded-xl bg-zinc-800 px-4 text-sm font-bold disabled:bg-zinc-700 disabled:text-zinc-300 disabled:opacity-100">{options.cancelLabel || "Cancel"}</button>}<button disabled={busy} onClick={submit} className={`min-h-11 rounded-xl px-4 text-sm font-black text-white ${destructive ? "bg-red-700" : "bg-orange-600"}`}>{busy ? "Confirming…" : label}</button></div></div></div>;
}
...
function OmluToast({ item, dismiss }: { item: { message: string; tone: Tone }; dismiss: () => void }) {
  ...
  const colors = { success: "border-emerald-800 bg-emerald-950 text-emerald-200", error: "border-red-800 bg-red-950 text-red-200", warning: "border-orange-800 bg-orange-950 text-orange-200", information: "border-zinc-700 bg-zinc-900 text-zinc-200" };
