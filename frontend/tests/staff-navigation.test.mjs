...
test("login and logout replace auth history entries", () => {
  ...
  assert.match(confirmedSignOut, /await staffLogout\(\)/);
  assert.match(api, /reason: "explicit_logout"[\s\S]*redirectTo: "\/login"/);
  assert.match(authRuntime, /runtime\.redirect\(destination\)/);
