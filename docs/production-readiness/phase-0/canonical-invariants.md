...
P1.1 implements this as frozen dataclasses `TenantScope` and `AuthenticatedContext` in `backend/app/utils/auth.py`. The external deterministic epoch representation is `<security_version>:<active_session_jti>`; it is not a process hash and is not accepted from request input.
