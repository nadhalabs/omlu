...
| Session bill | Staff | Send bill to counter | Draft/issued bill | `POST /staff/bills/{number}/send-to-counter` | Bill/session become `payment_pending` | Confirmation; duplicate guard; stale state refetch | `bill.sent_to_counter`, `bill.payment_pending` |
| Pending payments | Owner/Admin | Confirm Cash/UPI received | `payment_pending` bill | `POST /staff/bills/{number}/confirm-counter-payment` | Paid receipt, session closed, table free | Amount/table confirmation; one in-flight action; stale state refetch | `bill.payment_recorded`, `bill.paid`, `session.closed`, `table.status_changed` |
...
Staff never sees Cash/UPI controls or restaurant-wide payment history. Cash and UPI are manual Owner/Admin counter confirmations. Card, direct gateway, partial payment,
